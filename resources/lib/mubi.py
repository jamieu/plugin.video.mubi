...
class Mubi(object):
    ...
    def get_film_metadata(self, film_overview):
        ...
        self._film_info[film_id] = {"reel_id": film_overview['default_reel']['id'], "is_drm": drm}
    ...
    def now_showing(self):
        ...
        films = [self.get_film_metadata(film) for film in json.loads(r.text)]
    ...
    def get_play_url(self, film_id):
        args = "?client=android&country=GB&token=%s&udid=%s&client_version=3.05&film_id=%s&reel_id=%s&download=false" \
               % (self._token, self._udid, film_id, self._film_info[film_id]["reel_id"])
        ...
        item_result = {'url': url, 'is_mpd': "mpd" in url, 'is_drm': self._film_info[film_id]["is_drm"]}
